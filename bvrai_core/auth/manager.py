...
class AuthManager:
    ...
    def _hash_password(self, password: str) -> str:
        ...
        Uses cost factor of 12 (good balance of security and speed).
        """
        try:
            import bcrypt
            # bcrypt handles salt internally and returns hash with embedded salt
            return bcrypt.hashpw(password.encode('utf-8'), bcrypt.gensalt(rounds=12)).decode('utf-8')
        except ImportError:
            # Fallback to PBKDF2 if bcrypt not installed
            import hashlib
            import secrets
            salt = secrets.token_bytes(16)  # Use bytes, not hex string
            hash_val = hashlib.pbkdf2_hmac(
                'sha256',
                password.encode('utf-8'),
                salt,
                310000,  # OWASP recommended minimum iterations for PBKDF2-SHA256
            )
            # Store as: algorithm$iterations$salt$hash
            import base64
            return f"pbkdf2_sha256$310000${base64.b64encode(salt).decode()}${base64.b64encode(hash_val).decode()}"

    def _verify_password(self, password: str, password_hash: str) -> bool:
        """Verify a password against its hash using constant-time comparison.

        Supports both bcrypt and PBKDF2 hashes for backwards compatibility.
        """
        import hmac
        ...
        try:
            # Try bcrypt first (starts with $2b$ or $2a$)
